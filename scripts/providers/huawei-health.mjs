...
function normalizeHuaweiSession(raw, index, fallbackTimezone) {
  ...
  const distanceMeters = toNumber(
    pickFirst(raw.distanceMeters, raw.distance, raw.totalDistance, raw.totalDistanceMeters, raw.swimDistance),
  );
  const durationSeconds = toDurationSeconds(
    pickFirst(raw.durationSeconds, raw.duration, raw.totalTimeSeconds, raw.totalDuration, raw.durationMs),
  );
  const poolLengthMeters = toNumber(
    pickFirst(raw.poolLengthMeters, raw.poolLength, raw.laneLength, raw.laneDistanceMeters),
  );
  const derivedLaps =
    poolLengthMeters && distanceMeters ? Math.round(distanceMeters / poolLengthMeters) : undefined;
  const laps = toNumber(pickFirst(raw.laps, raw.lapCount, raw.lengths, raw.poolTrips, derivedLaps));
  ...
  const timezone = String(pickFirst(raw.timezone, raw.tz, raw.timeZone, fallbackTimezone));
  ...
  return {
    ...
    poolLengthMeters,
    laps,
    stroke: normalizeStroke(pickFirst(raw.stroke, raw.swimStyle, raw.strokeType)),
    swolf: toNumber(pickFirst(raw.swolf, raw.SWOLF, raw.avgSwolf)),
    calories: toNumber(pickFirst(raw.calories, raw.totalCalories, raw.energy)),
    notes: pickFirst(raw.notes, raw.note, raw.memo),
...
export async function loadHuaweiHealthProvider({ config, rootDir }) {
  ...
  if (mode !== "raw_json_file") {
    const missingCredentialEnvNames = [
      credentialsEnv.clientIdEnv,
      credentialsEnv.clientSecretEnv,
      credentialsEnv.redirectUriEnv,
    ].filter((envName) => envName && !process.env[envName]);

    if (missingCredentialEnvNames.length > 0) {
      warnings.push(`Missing Huawei Health env vars: ${missingCredentialEnvNames.join(", ")}`);
    }
  }
